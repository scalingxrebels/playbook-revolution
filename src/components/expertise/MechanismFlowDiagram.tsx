    ...
    <div ref={ref as React.RefObject<HTMLDivElement>} className="max-w-4xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        {mechanisms.map((m, i) => (
          ...
          >
            <div
              className={`p-6 md:p-8 border-2 bg-card transition-all ${
                activeM === m.id
                  ? 'border-primary shadow-glow'
                  : 'border-border'
              }`}
            >
              <span className={`text-xs font-bold tracking-wider block mb-3 ${
                activeM === m.id ? 'text-primary' : 'text-primary/60'
              }`}>
                {m.id}
              </span>
              <h4 className="font-display text-base md:text-lg font-bold text-foreground mb-2">
            ...
            {i < 2 && (
              <div className="hidden md:flex absolute -right-5 top-1/2 -translate-y-1/2 z-10">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
                  <path d="M3 12H21M21 12L14 5M21 12L14 19" stroke="hsl(var(--primary))" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" opacity={0.5} />
                </svg>
              </div>
            )}
            ...
            {i < 2 && (
              <div className="flex md:hidden justify-center py-3">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
                  <path d="M12 3V21M12 21L5 14M12 21L19 14" stroke="hsl(var(--primary))" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" opacity={0.5} />
                </svg>
              </div>
            )}
          </div>
      ...
      >
        ...
        <div className="hidden md:flex justify-around px-16 h-5">
          {[0, 1, 2].map((i) => (
            <svg key={i} width="2" height="20" viewBox="0 0 2 20" className="opacity-40">
              <line x1="1" y1="0" x2="1" y2="20" stroke="#F59E0B" strokeWidth="2" strokeDasharray="4 4" />
            </svg>
          ))}
        </div>

        <div
          className={`p-6 md:p-8 border-4 transition-all ${
            activeM === 'M4'
        ...
        >
          <div className="flex items-center justify-between flex-wrap gap-4">
            ...
            <span className="text-[10px] font-medium uppercase tracking-wider px-3 py-1.5 border-2 border-amber-500/30 text-amber-500/80">
