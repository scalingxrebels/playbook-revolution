        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
          {cases.map((c, i) => (
            <Link
              key={i}
              to={c.slug}
              className={`group block p-6 rounded-xl border-2 border-border/50 className={`group block p-6 rounded-xl border-2 border-border/50 bg-card hover:border-accent/50 hover:shadow-accent-glow transition-all duration-500 ${ hover:shadow-accent-glow transition-all duration-500 ${
                isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
              }`}
